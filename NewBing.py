...
class Conversation:
    ...
    def __init__(self, cookiePath: str = "", cookies: dict | None = None) -> None:
        ...
        if args.use_path_cookies:
            cookie_file = json.loads(os.environ['COOKIES'])
        else:
...
def save_datas(data1_list, data2_list, f):
    for i in range(0, len(data1_list)):
        save_data(data1_list[i], data2_list[i], f)
    ...
    while True:
        ...
        if question == "!exit":
            ...
            with open("data.md", "a")as f:
                save_data(old_question, response["item"]["messages"][1]["adaptiveCards"][0]["body"][0]["text"], f)
            continue
        elif question == "!saveall":
            with open("data.md", "a")as f:
                f.write("# "+time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())+"\n")
                save_datas(data1, data2, f)
            continue
        elif question == "!reset":
            await bot.reset()
            data1 = []
        ...
        data1.append(question)
        data2.append(response["item"]["messages"][1]["adaptiveCards"][0]["body"][0]["text"])
...
if __name__ == "__main__":
    ...
    if not args.enable:
        ...
        print("Using Setting")
    if setting.ask:
        args.enter_once = (input("enter_once(True/False): ") == "True")
        args.no_stream = (input("no_stream(True/False): ") == "True")
        args.rich = (input("rich(True/False): ") == "True")
        cookie_file = input("cookie_file: ")
        if cookie_file:
            args.cookie_file = cookie_file
    else:
        print("No ask")
    style = input("style(creative[1], balanced[2], precise[3]): ")
    if style:
        args.style = ["creative", "balanced", "precise"][int(style)-1]
    if not args.use_path_cookies:
